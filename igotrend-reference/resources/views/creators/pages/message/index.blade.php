                                ...
                                <div class="px-3 form-group mb-0">
                                    <div class="input-group input-group-merge input-group-rounded flex-nowrap">
                                        <select id="custom-select" class="form-control custom-select campaign_select">
                                            <option selected value="" data-url="{{route('creator_messages')}}">More Campaigns</option>
                                            @foreach($campaigns as $campaign)
                                                <option value="{{$campaign->id}}" @if($campaign_id == $campaign->id) selected @endif data-url="{{route('creator_messages',['campaign_id'=>$campaign->id])}}">{{$campaign->name}}</option>
                                            @endforeach
                                        </select>
