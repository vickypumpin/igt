...
<div class="mdk-drawer-layout js-mdk-drawer-layout" data-push data-responsive-width="992px">
    <div class="mdk-drawer-layout__content page-content">
        @include('creators.includes.header')
        @include('creators.includes.breadcrumb')
        @yield('content')
        @include('creators.includes.footer')
    </div>
    @include('creators.includes.sidebar')
</div>
<div id="app-settings">
...
@stack('modal')
@include('creators.includes.messages')
@stack('page-scripts')
<script>
    $("#unread_messages_button").on("click", function() {
        ...
        $.ajax({
            url: '{{route('get_creator_user_unread_messages')}}',
            type: "GET",
    ...
    $("#unread_notification_button").on("click", function() {
        ...
        $.ajax({
            url: '{{route('creator_user_unread_notifications')}}',
            type: "GET",
            success: function (data) {
                if(data.status){
    ...
    $(".read_all_notifications").on("click", function(e) {
        ...
        $('.total_notification_spinner').removeClass('d-none');
        $.ajax({
            url: '{{route('creator_read_all_notifications')}}',
            type: "GET",
            success: function (data) {
                if(data.status){
                    $('.total_notification_spinner').addClass('d-none');
...
<script>
    setInterval(function() {
        $.ajax({
            url: '{{route('creator_get_all_unread_messages')}}',
            type: "GET",
            success: function (data) {
                if(data.status){
                    //    messages
