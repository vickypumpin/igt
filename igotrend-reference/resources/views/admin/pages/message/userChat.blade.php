                ...
                <div class="col-md-8">
                    ...
                    <div class="card card-body text-center">
                        <div class="row" style="border-radius: 15px;">
                            <div class="col-md-12">
                                <div class="pt-3 px-2 messages_div" style="position: relative; max-height: 400px;  overflow-y:auto;">
                                    @if($messages->count() == 0)
                                        <h4 class="no_message_text">No Messages Found</h4>
                                    @else
                                        @foreach($messages as $message)
                                            @if($message->from_user == auth()->guard('admin')->user()->id)
                            ...
                            <form method="post" action="{{route('admin_send_msg')}}" class="ajax-form">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control"
                                              ...
                                              id="comment"
                                              rows="3"
                                              placeholder="Type here to reply to {{$receiver->first_name}} {{$receiver->last_name}}" required></textarea>
                                    <input type="hidden" name="from_user" value="{{auth()->guard('admin')->user()->id}}">
                                ...
                                <div class="px-3 form-group mb-0">
                                    <div class="input-group input-group-merge input-group-rounded flex-nowrap">
                                        <select id="custom-select" class="form-control custom-select campaign_select">
                                            <option selected value="" data-url="{{route('adminChatIndex',[$receiver])}}">More Campaigns</option>
                                            @foreach($campaigns as $campaign)
                                                <option value="{{$campaign->id}}" @if($campaign_id == $campaign->id) selected @endif data-url="{{route('adminChatIndex',[$receiver,'campaign_id'=>$campaign->id])}}">{{$campaign->name}}</option>
                                            @endforeach
                                        </select>
