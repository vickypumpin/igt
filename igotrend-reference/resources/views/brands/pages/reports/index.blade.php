...
@section('content')
    <style>
        ...
        .rating > input {display: none;}  /* Remove radio buttons */
        .rating > label:before {
            content: "\f005"; /* Star */
            margin: 1px;
    ...
    <div class="container-fluid page__container">
        <div class="page-section">
            ...
            <div class="row card-group-row mb-lg-8pt">
                ...
                <div class="col-lg-4 card-group-row__col">
                    <div class="card card-group-row__card align-items-center">
                        ...
                        <div class="card-body d-flex flex-column justify-content-center">
                            <div>
                                <a href="{{route('brand_reward_contacts')}}"><button type="button" class="btn btn-secondary">
            ...
            <div class="mb-2 mr-sm-2 mb-sm-0 flex align-items-right text-right">
                ...
                @if($campaign_id)
                    <a href="{{route('campaign_report',$campaign_id)}}" class="btn btn-sm btn-outline-secondary" >
                        Download
                        ...
                        <div class="px-3 form-group mb-0">
                            <div class="input-group input-group-merge input-group-rounded flex-nowrap">
                                <select  class="form-control" id="campaigns">
                                    <option value="" data-url="{{route('reports')}}">All Campaigns</option>
                                    @foreach($campaigns as $campaign)
                                        <option value="{{$campaign->id}}" data-url="{{route('reports',['campaign_id'=> $campaign->id])}}" @if($campaign_id == $campaign->id) selected @endif>
                                            {{$campaign->name}}
                                        </option>
                                    @endforeach
                                </select>
                                        ...
                                        <div class="posts-card__content d-flex align-items-center flex-wrap" data-toggle="collapse" data-target="#collapse{{$loop->iteration}}">
                                                ...
                                                <small class="text-50">
                                                    @if($submission->status == \App\Models\CampaignSubmission::REVIEW )
                                                        <a href="javascript:void(0)" class="btn btn-sm btn-secondary approve_submission"
                                                           data-url="{{route('approve_submission',$submission)}}">
                                                    ...
                                                    @if($submission->status == \App\Models\CampaignSubmission::REVIEW)
                                                        <a href="javascript:void(0)" class="btn btn-sm btn-secondary reject_submission"
                                                           data-url="{{route('reject_submission',$submission)}}">
                                            ...
                                            <div class="d-flex align-items-center flex-column flex-sm-row posts-card__meta">
                                                ...
                                                <div class="mr-3 text-50 posts-card__date">
                                                    @php
                                                        $check_review = \App\Models\CampaignSubmissionReview::where('submission_id',$submission->id)
                                                           ->where('from_user',auth()->user()->id)
                                                           ->where('to_user',$submission->user->id)
                                                           ->first();
                                                    @endphp
                                                    <div class="rating align-items-center flex-row-reverse justify-content-end">
                                                        <input type="radio" id="star5" name="rating" value="5" @if($check_review) @if($check_review->rating == 5) checked @endif @endif >
                                                        <label for="star5" class="rating_label"
                                                               data-rate="5"
                                                               data-user_id="{{$submission->user->id}}"
                                                               data-submission_id="{{$submission->id}}" title="5 star"
                                                               data-click="{{isset($check_review) ? true : false}}"
                                                        ></label>
                                                        <input type="radio" id="star4" name="rating" value="4" @if($check_review) @if($check_review->rating == 4) checked @endif @endif >
                                                        <label for="star4" class="rating_label" data-rate="4"
                                                               data-user_id="{{$submission->user->id}}"
                                                               data-submission_id="{{$submission->id}}"
                                                               title="4 star"
                                                               data-click="{{isset($check_review) ? true : false}}"
                                                        ></label>
                                                        <input type="radio" id="star3" name="rating" value="3" @if($check_review) @if($check_review->rating == 3) checked @endif @endif >
                                                        <label for="star3" class="rating_label"
                                                               data-rate="3"
                                                               data-user_id="{{$submission->user->id}}"
                                                               data-submission_id="{{$submission->id}}"
                                                               title="3 star"
                                                               data-click="{{isset($check_review) ? true : false}}"
                                                        ></label>
                                                        <input type="radio" id="star2" name="rating" value="2" @if($check_review) @if($check_review->rating == 2) checked @endif @endif >
                                                        <label for="star2" class="rating_label"
                                                               data-rate="2"
                                                               data-user_id="{{$submission->user->id}}"
                                                               data-submission_id="{{$submission->id}}"
                                                               title="2 star"
                                                               data-click="{{isset($check_review) ? true : false}}"
                                                        ></label>
                                                        <input type="radio" id="star1" name="rating" value="1" @if($check_review) @if($check_review->rating == 1) checked @endif @endif >
                                                        <label for="star1" class="rating_label"
                                                               data-rate="1"
                                                               data-user_id="{{$submission->user->id}}"
                                                               data-submission_id="{{$submission->id}}"
                                                               title="1 star"
...
@push('page-scripts')
    ...
    <script>
        ...
        $( ".rating_label" ).click(function() {
            const rating = $(this).data('rate');
            const user_id = $(this).data('user_id');
            const submission_id = $(this).data('submission_id');
