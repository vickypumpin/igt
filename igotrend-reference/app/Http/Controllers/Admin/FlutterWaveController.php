...
class FlutterWaveController extends Controller
{
    public function initialize(Campaign $campaign)
    {
        ...
        if ($amount == 0){
            ...
            $data = [
                ...
                'tx_ref' => $reference,
                'currency' => "NGN",
                'redirect_url' => route('admin-success-campaign-payment'),
                'customer' => [
                    'email' => $user->email,
    ...
    public function callback(){
        ...
        if ($status ==  'successful' || $status ==  'completed') {
            ...
            return redirect()->route('admin-payment-confirmation')->with('success','Payment done Successfully');
        } elseif ($status ==  'cancelled'){
            return redirect()->route('manage_admin_campaigns')->with('danger','Sorry your payment cancelled');
        } else{
            return redirect()->route('manage_admin_campaigns')->with('danger','Something went wrong');
        }
    }
    public function success(){
        return view('admin.pages.campaign.payment_confirmation');
    }
    public function payRequestUser(CampaignInvite $invite)
    {
        ...
        if ($amount == 0){
            ...
            $data = [
                ...
                'tx_ref' => $reference,
                'currency' => "NGN",
                'redirect_url' => route('successRequestPayment'),
                'customer' => [
                    'email' => $user->email,
                    "name" => $user->first_name.' ',$user->last_name
                ],
    ...
    public function successRequestPayment(){
        $status = request()->status;
        //if payment is successful
        if ($status ==  'successful' || $status ==  'completed') {
            $transactionID = Flutterwave::getTransactionIDFromCallback();
...
//            }
            ...
            return redirect()->route('adminDashboard')->with('success','Payment done Successfully');
        } elseif ($status ==  'cancelled'){
            return redirect()->route('adminDashboard')->with('danger','Sorry your payment cancelled');
        } else{
            return redirect()->route('adminDashboard')->with('danger','Something went wrong');
        }
    }
    public function GemPaymentSuccess(){
        ...
        if ($status ==  'successful' || $status ==  'completed') {
            ...
            return redirect()->route('admin_reward_contacts')->with('success','Reward Send Successfully');
        } elseif ($status ==  'cancelled'){
            return redirect()->route('admin_reward_contacts')->with('danger','Sorry your payment cancelled');
        } else{
            return redirect()->route('admin_reward_contacts')->with('danger','Something went wrong');
        }
    }
