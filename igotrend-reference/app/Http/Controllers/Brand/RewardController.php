...
class RewardController extends Controller
{
    ...
    {
        $brand_campaigns = Campaign::where('user_id',auth()->user()->id)
            ->where('status',Campaign::ACTIVE)
            ->orwhere('status',Campaign::COMPLETED)
            ->get();
        $setting = Setting::first();

        return view('brands.pages.reward.index',compact('setting','brand_campaigns'));
    }
            ...
            if ($request->payment_type == "mobile_airtime"){
                ...
                if ($total_pay_with_percentage > 0){
                    ...
                    $data = [
                        ...
                        'tx_ref' => $reference,
                        'currency' => "NGN",
                        'redirect_url' => route('trenderAirTimePaymentSuccess'),
                        'customer' => [
                            'email' => auth()->user()->email,
            ...
            if ($request->payment_type == "gems"){
                ...
                if ($total_pay_with_percentage > 0){
                    ...
                    $data = [
                        ...
                        'tx_ref' => $reference,
                        'currency' => "NGN",
                        'redirect_url' => route('trenderGemPaymentSuccess'),
                        'customer' => [
                            'email' => auth()->user()->email,
                            "phone_number" => auth()->user()->phone,
                            "name" => auth()->user()->first_name.' ',auth()->user()->last_name
    ...
    public function get_brand_campaign_users(Campaign $campaign)
    {
                                                                    ...
                                                                    <div class="media-body">
                                                                        <div class="d-flex align-items-center">
                                                                            <div class="flex d-flex flex-column">
                                                                                <a data-url="'.route('getTrenderInfo',$reward->reward_user).'" class="view_trender_info" class="mb-0" style="cursor: pointer">
