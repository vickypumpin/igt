...
class CampaignController extends Controller
{
    ...
    {
        $active_campaigns = Campaign::where('status',Campaign::ACTIVE)
            ->where('user_id',auth()->user()->id)
            ->count();
        $pending_campaigns = Campaign::where('status',Campaign::PENDING)
            ->where('user_id',auth()->user()->id)
            ->count();
        $decline_campaigns = Campaign::where('status',Campaign::DECLINE)
            ->where('user_id',auth()->user()->id)
            ->count();
        $completed_campaigns = Campaign::where('status',Campaign::COMPLETED)
            ->where('user_id',auth()->user()->id)
            ->count();
        $setting = Setting::first();
        return view('brands.pages.campaign.my_campaign',
            compact('active_campaigns','pending_campaigns','decline_campaigns','completed_campaigns','setting'));
    }
        ...
        if($request->ajax()){
            ...
            return DataTables::of($campaigns)
                ->addColumn('actions', function ($campaign){
                    ...
                    $html .= '<a href="'.route('campaign_detail',$campaign).'" class="text-50">
                ...
                ->addColumn('project', function ($campaign){
                    ...
                    return '<div class="media flex-nowrap align-items-center" style="white-space: nowrap;">
                                                                    ...
                                                                    <a  href="'.route('campaign_detail',$campaign).'" class="">
                ...
                ->setRowId(function ($campaign) {
                    return route('campaign_detail',$campaign);
                })
                ->make(true);
        }
    }
    ...
    public function create_campaign()
    {
        ...
        return view('brands.pages.campaign.create_campaign',compact('content_categories','creator_categories'));
    }
    public function create_campaign_save(Request $request)
    {
        ...
        return redirect()->route('campaign_brief',$campaign);
    }
    public function campaign_brief(Campaign $campaign)
    {
        return view('brands.pages.campaign.campaign_brief',compact('campaign'));
    }
    public function campaign_brief_save(Request $request,Campaign $campaign)
    {
        ...
        return redirect()->route('add_trenders',$campaign);
    }
    public function add_trenders(Request $request,Campaign $campaign)
    {
        ...
        return view('brands.pages.campaign.add_trenders',compact('campaign','countries',
            'trenders',
            'campaign_users',
    ...
    public function add_trenders_save(Request $request,Campaign $campaign)
    {
        ...
        return redirect()->route('review_payment',$campaign);
    }
    public function review_payment(Campaign $campaign)
    {
        ...
        return view('brands.pages.campaign.review_payment',compact('campaign','count_posts_story','estimated_budget'));
    }
    public function deleteCampaign(Campaign $campaign)
    {
        ...
        return response()->json([
            'status' => true,
            'redirect_url' => route('my_campaigns')
        ]);
    }
    public function disableCampaign(Campaign $campaign)
    {
        $campaign->update([
            'is_disable' => Campaign::DISABLE
        ]);
        $message = 'Campaign Disable Successfully';
        return redirect()->back()->with('success',$message);
    }
    public function enableCampaign(Campaign $campaign)
    {
        $campaign->update([
            'is_disable' => Campaign::ENABLE
        ]);
        $message = 'Campaign Enable Successfully';
        return redirect()->back()->with('success',$message);
    }
    ...
    public function campaign_detail(Campaign $campaign)
    {
        $count_posts_story = Campaign::calculate_total_deliverables($campaign);
        $budget = Campaign::calculate_estimated_price($campaign);
        return view('brands.pages.campaign.detail',compact('campaign','count_posts_story','budget'));
    }
    ...
    public function edit_campaign(Campaign $campaign)
    {
        ...
        return view('brands.pages.campaign.edit_campaign',compact('campaign','content_categories','creator_categories','age_range'));
    }
    ...
    public function trenderInvitesRequest(Request $request)
    {
        if($request->ajax()){
            ...
            return DataTables::of($invitations)
                ->addColumn('actions', function ($invite){
                    ...
                    $html .= '<a href="#"  class="btn btn-sm btn-primary view_trender_info" data-url="'.route('getUserInfo',[$invite->user,$invite->campaign]).'">View</a>';
                    if ($invite->payment_status == "paid"){
                        $html .= '<a href="'.route('accept_trender_invitation',$invite).'" class="btn btn-sm btn-success">Accept</a>';
                    }else{
                        $html .= '<a href="'.route('payTrender',$invite).'" class="btn btn-sm btn-success">Accept / Pay</a>';
                    }

                    $html .= '<a href="'.route('decline_trender_invitation',$invite).'" class="btn btn-danger btn-sm">Decline</a>';
                ...
                ->addColumn('trender', function ($invite){
                    ...
                    return '<div class="media flex-nowrap align-items-center" style="white-space: nowrap;">
                                    <div class="avatar avatar-32pt mr-8pt">
                                        <a href="#" class="view_trender_info" data-url="'.route('getUserInfo',[$invite->user,$invite->campaign]).'">
                                           <span class="avatar-title rounded-circle">'.$acronym.'</span>
                                        </a>
                                    </div>
                                    <div class="media-body">
...
//                    }
    ...
    public function accept_trender_invitation(CampaignInvite $invite)
    {
        $invite->update([
    ...
    public function decline_trender_invitation(CampaignInvite $invite)
    {
        $invite->update([
