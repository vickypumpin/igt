...
class ReportController extends Controller
{
    public function reports(Request $request)
    {
        ...
        $reward_send = UserReward::where('from_user',auth()->user()->id)->sum('amount');
        ...
        $campaign_budget = Payment::where('user_id',auth()->user()->id)
            ->whereIn('campaign_id',$completed_campaigns)
        ...
        $trender_collabs = CampaignInvite::whereIn('campaign_id',$completed_campaigns)
            ...
            ->count();
        $reach = CampaignSubmission::whereIn('campaign_id',$completed_campaigns)
            ->sum('views');
        $engagement  = CampaignSubmission::whereIn('campaign_id',$completed_campaigns)
            ->sum('likes');

        return view('brands.pages.reports.index',compact('campaigns','campaigns_submissions','campaign_id',
...
//            }
    ...
    {
        CampaignSubmissionReview::create([
    ...
    public function campaign_report($campaign_id)
    {
        ...
        if ($campaign){
            ...
            return view('brands.pages.reports.report_download',compact('campaign','budget','total_reach','total_engagement','count_posts_story'));
        }else{
