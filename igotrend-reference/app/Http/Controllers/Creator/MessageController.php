...
class MessageController extends Controller
{
    public function index(Request $request)
    {
        ...
        $invites_id = CampaignInvite::where('user_id',auth()->user()->id)
            ->where('status',CampaignInvite::ACTIVE)
            ->pluck('campaign_id');
        $campaigns = Campaign::whereIn('id',$invites_id)
            ->where('status',Campaign::ACTIVE)
            ->orderBy('id', 'DESC')
            ->get();
        $advertisers_ids = Campaign::whereIn('id',$invites_id)
            ->pluck('user_id');
        ...
        return view('creators.pages.message.index',compact(
            'campaigns',
            'campaign_id',
            'advertisers',
            'admins',
    ...
    public function chatIndex(User $receiver,Request $request)
    {
        ...
        $campaigns = Campaign::whereIn('id',$invities_id)
            ->orderBy('id', 'DESC')
            ->get();
        $advertisers_ids = Campaign::whereIn('id',$invities_id)
            ->pluck('user_id');
        ...
        return view('creators.pages.message.userChat',compact('campaigns',
            'campaign_id',
            'advertisers',
            'receiver',
            'messages',
    ...
    public function send_msg(Request $request)
    {
        ...
        $message_html = view('creators.pages.message.message',compact('message'))->render();
        ...
        return response()->json([
            ...
            'img_path' => get_user_image($message->from_user),
            'date_time' => \Carbon\Carbon::parse($message->created_at)->format('H:i A') .'|'. $message->created_at->format('d M'),
        ]);
    }
    public function admin_chat(Admin $receiver,Request $request)
    {
        ...
        return view('creators.pages.message.adminChat',compact('campaigns',
            'campaign_id',
            'advertisers',
            'receiver',
            'messages',
    ...
    public function get_user_unread_messages()
    {
        ...
        foreach ($un_read_message as $message){
            $is_admin = false;
            $render_message =  view('creators.pages.message.header_message',compact('message','is_admin'))->render();
            $messages .= $render_message;
        }

        foreach ($admin_un_read_message as $admin_message){
            $is_admin = true;
            $render_message =  view('creators.pages.message.header_message',compact('admin_message','is_admin'))->render();
            $messages .= $render_message;
        }
